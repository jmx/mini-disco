...
fn context_error(stage: &'static str, err: impl Display) -> io::Error {
    io::Error::other(format!("{stage}: {err}"))
}
...
pub enum InterfaceError {
    ...
    #[error("the device rejected the message: {0}")]
    Rejected(String),
...
impl EKBOpenSource {
    pub fn ekb_data_for_device(&self, leaf_id: [u8; 8], vendor_id: u16, product_id: u16) -> EKBData {
        if leaf_id.iter().all(|byte| *byte == 0xff) && vendor_id == 0x054c && product_id == 0x0081
        {
            return EKBData {
                root_key: [
                    0x57, 0x4d, 0x44, 0x50, 0x57, 0x4d, 0x44, 0x50, 0x4d, 0x69, 0x6e, 0x69,
                    0x44, 0x69, 0x73, 0x63,
                ],
                ekb_id: 0x13371337,
                chains: vec![[
                    0xb1, 0xd4, 0xaf, 0xfa, 0x80, 0xa0, 0xc9, 0x03, 0xc2, 0x58, 0x4b, 0x1b,
                    0x44, 0xaf, 0xc4, 0xa6,
                ]],
                depth: 9,
                signature: [
...
impl<'a> MDSession<'a> {
    pub async fn init(&mut self) -> Result<(), Box<dyn Error>> {
        self.md
            ...
            .await
            .map_err(|err| context_error("enter secure session failed", err))?;
        let leaf_id = self
            .md
            .leaf_id()
            .await
            .map_err(|err| context_error("read leaf id failed", err))?;

        let ekb = self.ekb_object.ekb_data_for_device(
            leaf_id,
            self.md.device.vendor_id(),
            self.md.device.product_id(),
        );
        self.md
            .send_key_data(ekb.ekb_id, ekb.chains, ekb.depth, ekb.signature)
            .await
            .map_err(|err| context_error("send key data failed", err))?;
        let mut nonce = vec![0u8; 8];
        rand::thread_rng().fill_bytes(&mut nonce);

        let mut devnonce = self
            .md
            .session_key_exchange(nonce.clone())
            .await
            .map_err(|err| context_error("session key exchange failed", err))?;
        nonce.append(&mut devnonce);

        self.hex_session_key = Some(retailmac(&ekb.root_key, &nonce, &[0u8; 8]));
        Ok(())
    }
    ...
    ) -> Result<(u16, Vec<u8>, Vec<u8>), Box<dyn Error>>
    {
        ...
        self.md
            ...
            .await
            .map_err(|err| context_error("setup download failed", err))?;
        ...
        let (track_index, uuid, ccid) = self
            ...
            .await
            .map_err(|err| context_error("send track failed", err))?;

        self.md
            .set_track_title(track_index, &track.title, false)
            .await
            .map_err(|err| context_error("set track title failed", err))?;
        if let Some(full_width) = track.full_width_title {
            self.md
                .set_track_title(track_index, &full_width, true)
                .await
                .map_err(|err| context_error("set full-width track title failed", err))?;
        }
        self.md
            .commit_track(track_index, self.hex_session_key.as_ref().unwrap())
            .await
            .map_err(|err| context_error("commit track failed", err))?;
